    ...
    public class SqlMap<TKey, TValue> : MapStorageBase<TKey, TValue>, IKeysMap<TKey, TValue>, IPropertyMap<TKey, TValue>, IUpdatableMap<TKey, TValue>
    {
            ...
            using (var cancellationTokenSource = CreateCancellationTokenSource())
            {
                using (var connection = await GetConnectionAsync(cancellationTokenSource.Token).ConfigureAwait(false))
                {
                    ...
                    using (var command = overwrite ? 
                        connection.CreateMergeCommand(DatabaseDriver, Table.Schema, Table.Name, keyColumnValues, columnValues, Table.Columns) : 
                        connection.CreateInsertWhereNotExistsCommand(DatabaseDriver, Table.Schema, Table.Name, keyColumnValues, columnValues, Table.Columns))
                    {
            ...
            using (var cancellationTokenSource = CreateCancellationTokenSource())
            {
                using (var connection = await GetConnectionAsync(cancellationTokenSource.Token).ConfigureAwait(false))
                {
                    ...
                    return await new DbDataReaderAsyncEnumerable<TValue>(
                        ...
                        c => c.CreateSelectCommand(DatabaseDriver, Table.Schema, Table.Name, keyColumnValues, Table.Columns, selectColumns),
                        Mapper,
                        selectColumns)
        ...
        public virtual Task<IAsyncEnumerable<TKey>> GetKeysAsync()
        {
            ...
            return Task.FromResult<IAsyncEnumerable<TKey>>(
                new DbDataReaderAsyncEnumerable<TKey>(GetConnectionAsync, c => c.CreateSelectCommand(DatabaseDriver, Table.Schema, Table.Name, null, Table.Columns, selectColumns), KeyMapper, selectColumns));
        }
        ...
        public virtual async Task SetPropertyValueAsync<TProperty>(TKey key, string propertyName, TProperty propertyValue)
        {
            ...
            using (var cancellationTokenSource = CreateCancellationTokenSource())
            {
                using (var connection = await GetConnectionAsync(cancellationTokenSource.Token).ConfigureAwait(false))
                {
                    ...
                    using (var command = connection.CreateMergeCommand(DatabaseDriver, Table.Schema, Table.Name, keyColumnValues, columnValues, Table.Columns))
                    {
        ...
        public virtual async Task<TProperty> GetPropertyValueOrDefaultAsync<TProperty>(TKey key, string propertyName)
        {
            ...
            using (var cancellationTokenSource = CreateCancellationTokenSource())
            {
                using (var connection = await GetConnectionAsync(cancellationTokenSource.Token).ConfigureAwait(false))
                {
                    ...
                    using (var command = connection.CreateSelectTop1Command(DatabaseDriver, Table.Schema, Table.Name, new[] { propertyName }, keyColumnValues, Table.Columns))
                    {
