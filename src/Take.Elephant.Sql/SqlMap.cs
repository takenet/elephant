...
namespace Take.Elephant.Sql
{
    ...
    {
        private readonly IDbConnectionExtensionInstatiable _dbConnectionExtensionInstatiable;

        public SqlMap(string connectionString, ITable table, IMapper<TKey> keyMapper, IMapper<TValue> valueMapper,IDbConnectionExtensionInstatiable dbConnectionExtensionInstatiable)
            : this(new SqlDatabaseDriver(), connectionString, table, keyMapper, valueMapper)
        {
            _dbConnectionExtensionInstatiable = dbConnectionExtensionInstatiable;
        }
            ...
            using (var cancellationTokenSource = CreateCancellationTokenSource())
            {
                using (var connection = await GetConnectionAsync(cancellationTokenSource.Token).ConfigureAwait(false))
                {
                    ...
                    using (var command = overwrite ? 
                        ConnectionExtensions.CreateMergeCommand(connection, DatabaseDriver, Table, keyColumnValues, columnValues) : 
                        ConnectionExtensions.CreateInsertWhereNotExistsCommand(connection, DatabaseDriver, Table, keyColumnValues, columnValues))
                    {
        ...
        public virtual Task<IAsyncEnumerable<TKey>> GetKeysAsync()
        {
            ...
            return Task.FromResult<IAsyncEnumerable<TKey>>(
                new DbDataReaderAsyncEnumerable<TKey>(GetConnectionAsync, c => ConnectionExtensions.CreateSelectCommand(c, DatabaseDriver, Table, null, selectColumns), KeyMapper, selectColumns));
        }
            ...
            TProperty propertyValue, CancellationToken cancellationToken = default)
        {
            ...
            using (var cancellationTokenSource = CreateCancellationTokenSource())
            {
                using (var connection = await GetConnectionAsync(cancellationTokenSource.Token).ConfigureAwait(false))
                {
                    ...
                    using (var command = ConnectionExtensions.CreateMergeCommand(connection, DatabaseDriver, Table, keyColumnValues, columnValues))
                    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            using (var cancellationTokenSource = CreateCancellationTokenSource())
            {
                using (var connection = await GetConnectionAsync(cancellationTokenSource.Token).ConfigureAwait(false))
                {
                    ...
                    using (var command = _dbConnectionExtensionInstatiable.CreateSelectTop1Command(connection, DatabaseDriver, Table, new[] { propertyName }, keyColumnValues))
                    {
        ...
        public virtual async Task<bool> TryUpdateAsync(TKey key, TValue newValue, TValue oldValue)
        {
            using (var cancellationTokenSource = CreateCancellationTokenSource())
            {
                using (var connection = await GetConnectionAsync(cancellationTokenSource.Token).ConfigureAwait(false))
                {
                    ...
                    using (var command = ConnectionExtensions.CreateTextCommand(
                        connection,
