...
namespace Take.Elephant.Azure
{
    ...
    internal class AzureEventHubReceiverQueue<T> : IReceiverQueue<T>, IBlockingReceiverQueue<T>, IBatchReceiverQueue<T>, ICloseable, IAsyncDisposable
    {
        ...
        public async IAsyncEnumerable<T> DequeueBatchAsync(int maxBatchSize, CancellationToken cancellationToken)
        {
            await StartConsumerTaskIfNotAsync(cancellationToken);
