...
namespace Take.Elephant.Tests.Kafka.SchemaRegistry.Avro
{
    ...
    public class KafkaSchemaRegistryAvroEventStreamPublisherConsumerFacts : IDisposable
    {
        private readonly string _bootstrapServers = "localhost:9092";
        private readonly string _schemaRegistryUrl = "http://localhost:8081";
        private readonly CancellationTokenSource _cts;

        public KafkaSchemaRegistryAvroEventStreamPublisherConsumerFacts()
        {
        ...
        {
            var topic = "test-avro-stream-" + Guid.NewGuid().ToString("N");
            ...
            using var publisher = new KafkaSchemaRegistryEventStreamPublisher<string, SchemaRegistryAvroTestItem>(
                producerConfig, topic, schemaRegistryOptions);
            using var consumer = new KafkaSchemaRegistryEventStreamConsumer<string, SchemaRegistryAvroTestItem>(
        ...
        {
            var topic = "test-avro-stream-multi-" + Guid.NewGuid().ToString("N");
            var groupId = Guid.NewGuid().ToString();
            var schemaRegistryOptions = CreateSchemaRegistryOptions();
            ...
            var items = Enumerable.Range(1, 2).Select(i => new SchemaRegistryAvroTestItem
            {
                Id = Guid.NewGuid().ToString(),
                Name = $"Avro Test Item {i}",
                Value = i * 10,
                CreatedAt = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()
            }).ToList();
        ...
        {
            var topic = "test-avro-stream-shared-" + Guid.NewGuid().ToString("N");
            var groupId = Guid.NewGuid().ToString();
            var schemaRegistryOptions = CreateSchemaRegistryOptions();
