...
namespace Take.Elephant.Tests.Specialized
{
    public abstract class DistributedCacheSetMapFacts<TKey, TValue> : DistributedCacheMapFacts<TKey, ISet<TValue>>
    {
        ...
        [Fact(DisplayName = nameof(AddToAllSetsShouldSynchronizeTheCache))]
        public async Task AddToAllSetsShouldSynchronizeTheCache()
        {
            // Arrange
        ...
        [Fact(DisplayName = nameof(AddToOneSetShouldSynchronizeTheCache))]
        public async Task AddToOneSetShouldSynchronizeTheCache()
        {
            // Arrange
            var source = CreateSource();
            var cache = CreateCache();
            ...
            // Act
            await target1.AddItemAsync(key, item1);
            await target1.AddItemAsync(key, item2);
